...
const categoryColors: Record<string, string> = {
  ...
  FAN_OPINION: 'bg-pink-100 text-pink-800',
  ...
  return (
    <article className="bg-white shadow rounded-lg p-4 flex flex-col gap-2">
      <div className="flex items-center gap-2">
        <span
          className={`px-2 py-0.5 rounded text-xs font-semibold uppercase ${categoryColors[news.category]}`}
        >
      ...
      {news.imageUrl && (
        <img src={news.imageUrl} alt={news.title} className="w-full h-48 object-cover rounded mb-2" />
      )}
